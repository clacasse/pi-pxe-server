#!/bin/bash
# Runs once on the PXE server Pi's first boot via cloud-init runcmd.
# Sets up a PXE server for x86_64 Ubuntu autoinstall.
set -e
...
for BOOT_REPO in /boot/firmware/node-bootstrap /boot/node-bootstrap; do
    [ -d "$BOOT_REPO" ] && break
done
echo "Repo: $BOOT_REPO"
...
sed "s|__PI_IP__|${PI_IP}|g" "$BOOT_REPO/templates/grub-x86_64.cfg.tpl" > /srv/tftp/grub/grub.cfg
...
wget -q --show-progress -O /srv/http/ubuntu.iso \
...
[ -f /srv/tftp/grubnetx64.efi ] && echo "GRUB: ready"

echo "=== PXE Setup Complete $(date) ==="
echo ""
echo "PXE server ready at $PI_IP"
echo "Any x86_64 machine that PXE boots will get Ubuntu installed automatically."
